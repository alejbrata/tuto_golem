...
function CharacterCreator({ onComplete }) {
    ...
    return (
        <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4 relative overflow-hidden">
            ...
            <div className="relative z-10 w-full max-w-md bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-2xl p-8 shadow-2xl">
                ...
                <p className="text-center text-slate-400 text-sm mb-8">
                    The DNA of your Construct determines its affinity.
                </p>
                ...
                <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4 text-xs font-mono text-slate-500 mb-4">
                        <div className="bg-slate-950 p-2 rounded border border-slate-800">
                            TYPE: <span className="text-slate-300">{['SENTINEL', 'GUARDIAN', 'CONSTRUCT'][decoded.bodyType]}</span>
                        </div>
                        <div className="bg-slate-950 p-2 rounded border border-slate-800">
                            CORE: <span className={`text-${decoded.color.tw}-400`}>{decoded.color.name.toUpperCase()}</span>
                        </div>
                    </div>
